...
public class Grid : MonoBehaviour
{
    ...
    void calculatePathDijkstra(Vector2Int start, Vector2Int finish)
    {
        //  Очищаем все узлы - сбрасываем отметку родителя, снимаем подсветку
        foreach (var node in grid)
        {
        ...
        while (nodes.Count != 0)
        {
            ...
            foreach (var neighbourGrid in neighbours)
            {
                var neighbourNode = grid[neighbourGrid.x, neighbourGrid.y];
    ...
    void calculatePathAStar(Vector2Int start, Vector2Int finish)
    {
        //  Очищаем все узлы - сбрасываем отметку родителя, снимаем подсветку
        foreach (var node in grid)
        {
